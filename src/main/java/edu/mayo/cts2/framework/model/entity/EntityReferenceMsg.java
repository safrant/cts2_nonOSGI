/*
 * This class was automatically generated with 
 * <a href="http://www.castor.org">Castor 1.3.1</a>, using an XML
 * Schema.
 * $Id$
 */

package edu.mayo.cts2.framework.model.entity;

import edu.mayo.cts2.framework.model.core.EntityReference;
import edu.mayo.cts2.framework.model.core.Message;

/**
 * An EntityReference as returned from a service
 * 
 * @version $Revision$ $Date$
 */
@SuppressWarnings("serial")
public class EntityReferenceMsg extends Message
implements java.io.Serializable
{


      //--------------------------/
     //- Class/Member Variables -/
    //--------------------------/

    /**
     * Field _entityReference.
     */
    private EntityReference _entityReference;


      //----------------/
     //- Constructors -/
    //----------------/

    public EntityReferenceMsg() {
        super();
    }


      //-----------/
     //- Methods -/
    //-----------/

    /**
     * Overrides the java.lang.Object.equals method.
     * 
     * @param obj
     * @return true if the objects are equal.
     */
    @Override()
    public boolean equals(
            final Object obj) {
        if ( this == obj )
            return true;

        if (!super.equals(obj))
            return false;

        if (obj instanceof EntityReferenceMsg) {

            EntityReferenceMsg temp = (EntityReferenceMsg)obj;
            if (this._entityReference != null) {
                if (temp._entityReference == null) return false;
                return this._entityReference.equals(temp._entityReference);
            } else return temp._entityReference == null;
        }
        return false;
    }

    /**
     * Returns the value of field 'entityReference'.
     * 
     * @return the value of field 'EntityReference'.
     */
    public EntityReference getEntityReference(
    ) {
        return this._entityReference;
    }

    /**
     * Overrides the java.lang.Object.hashCode method.
     * <p>
     * The following steps came from <b>Effective Java Programming
     * Language Guide</b> by Joshua Bloch, Chapter 3
     * 
     * @return a hash code value for the object.
     */
    public int hashCode(
    ) {
        int result = super.hashCode();

        long tmp;
        if (_entityReference != null) {
           result = 37 * result + _entityReference.hashCode();
        }

        return result;
    }

    /**
     * Method isValid.
     * 
     * @return true if this object is valid according to the schema
     */

    /**
     * 
     * 
     * @param out
     * @throws org.exolab.castor.xml.MarshalException if object is
     * null or if any SAXException is thrown during marshaling
     * @throws org.exolab.castor.xml.ValidationException if this
     * object is an invalid instance according to the schema
     */

    /**
     * 
     * 
     * @param handler
     * @throws java.io.IOException if an IOException occurs during
     * marshaling
     * @throws org.exolab.castor.xml.ValidationException if this
     * object is an invalid instance according to the schema
     * @throws org.exolab.castor.xml.MarshalException if object is
     * null or if any SAXException is thrown during marshaling
     */

    /**
     * Sets the value of field 'entityReference'.
     * 
     * @param entityReference the value of field 'entityReference'.
     */
    public void setEntityReference(
            final EntityReference entityReference) {
        this._entityReference = entityReference;
    }

    /**
     * Method unmarshalEntityReferenceMsg.
     * 
     * @param reader
     * @throws org.exolab.castor.xml.MarshalException if object is
     * null or if any SAXException is thrown during marshaling
     * @throws org.exolab.castor.xml.ValidationException if this
     * object is an invalid instance according to the schema
     * @return the unmarshaled
     * edu.mayo.cts2.framework.model.entity.EntityReferenceMsg
     */

    /**
     * 
     * 
     * @throws org.exolab.castor.xml.ValidationException if this
     * object is an invalid instance according to the schema
     */

}
