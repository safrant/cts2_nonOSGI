/*
 * This class was automatically generated with 
 * <a href="http://www.castor.org">Castor 1.3.1</a>, using an XML
 * Schema.
 * $Id$
 */

package edu.mayo.cts2.framework.model.service.mapentry;

/**
 * Class UpdatedExternalEntryId.
 * 
 * @version $Revision$ $Date$
 */
@SuppressWarnings("serial")
public class UpdatedExternalEntryId extends edu.mayo.cts2.framework.model.BaseCts2ModelObject 
implements java.io.Serializable
{


      //--------------------------/
     //- Class/Member Variables -/
    //--------------------------/

    /**
     * an external identifier assigned to this target
     *  entry by an outside body
     */
    private String _externalEntryId;


      //----------------/
     //- Constructors -/
    //----------------/

    public UpdatedExternalEntryId() {
        super();
    }


      //-----------/
     //- Methods -/
    //-----------/

    /**
     * Overrides the java.lang.Object.equals method.
     * 
     * @param obj
     * @return true if the objects are equal.
     */
    @Override()
    public boolean equals(
            final Object obj) {
        if ( this == obj )
            return true;

        if (!super.equals(obj))
            return false;

        if (obj instanceof UpdatedExternalEntryId) {

            UpdatedExternalEntryId temp = (UpdatedExternalEntryId)obj;
            if (this._externalEntryId != null) {
                if (temp._externalEntryId == null) return false;
                return this._externalEntryId.equals(temp._externalEntryId);
            } else return temp._externalEntryId == null;
        }
        return false;
    }

    /**
     * Returns the value of field 'externalEntryId'. The field
     * 'externalEntryId' has the following description: an external
     * identifier assigned to this target
     *  entry by an outside body
     * 
     * @return the value of field 'ExternalEntryId'.
     */
    public String getExternalEntryId(
    ) {
        return this._externalEntryId;
    }

    /**
     * Overrides the java.lang.Object.hashCode method.
     * <p>
     * The following steps came from <b>Effective Java Programming
     * Language Guide</b> by Joshua Bloch, Chapter 3
     * 
     * @return a hash code value for the object.
     */
    public int hashCode(
    ) {
        int result = super.hashCode();

        long tmp;
        if (_externalEntryId != null) {
           result = 37 * result + _externalEntryId.hashCode();
        }

        return result;
    }

    /**
     * Method isValid.
     * 
     * @return true if this object is valid according to the schema
     */

    /**
     * 
     * 
     * @param out
     * @throws org.exolab.castor.xml.MarshalException if object is
     * null or if any SAXException is thrown during marshaling
     * @throws org.exolab.castor.xml.ValidationException if this
     * object is an invalid instance according to the schema
     */

    /**
     * 
     * 
     * @param handler
     * @throws java.io.IOException if an IOException occurs during
     * marshaling
     * @throws org.exolab.castor.xml.ValidationException if this
     * object is an invalid instance according to the schema
     * @throws org.exolab.castor.xml.MarshalException if object is
     * null or if any SAXException is thrown during marshaling
     */

    /**
     * Sets the value of field 'externalEntryId'. The field
     * 'externalEntryId' has the following description: an external
     * identifier assigned to this target
     *  entry by an outside body
     * 
     * @param externalEntryId the value of field 'externalEntryId'.
     */
    public void setExternalEntryId(
            final String externalEntryId) {
        this._externalEntryId = externalEntryId;
    }

    /**
     * Method unmarshalUpdatedExternalEntryId.
     * 
     * @param reader
     * @throws org.exolab.castor.xml.MarshalException if object is
     * null or if any SAXException is thrown during marshaling
     * @throws org.exolab.castor.xml.ValidationException if this
     * object is an invalid instance according to the schema
     * @return the unmarshaled
     * edu.mayo.cts2.framework.model.service.mapentry.UpdatedExternalEntryId
     */

    /**
     * 
     * 
     * @throws org.exolab.castor.xml.ValidationException if this
     * object is an invalid instance according to the schema
     */

}
